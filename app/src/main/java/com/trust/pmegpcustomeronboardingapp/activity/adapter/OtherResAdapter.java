...
public class OtherResAdapter extends RecyclerView.Adapter<OtherResAdapter.ClassViewHolder> {

    private List<ScoreCard.ScoreParameter> classList;
    private int selectedPosition = -1;
    private OnDocUploadClickListener uploadClickListener;
    public OtherResAdapter(List<ScoreCard.ScoreParameter> classList, OnDocUploadClickListener listener) {
    ...
    @NonNull
    @Override
    public OtherResAdapter.ClassViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.adapter_applicant_age_item, parent, false);
        return new OtherResAdapter.ClassViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull OtherResAdapter.ClassViewHolder holder, @SuppressLint("RecyclerView") int position) {
