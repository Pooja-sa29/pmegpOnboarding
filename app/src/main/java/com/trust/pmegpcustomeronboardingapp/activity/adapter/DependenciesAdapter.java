...
public class DependenciesAdapter extends RecyclerView.Adapter<DependenciesAdapter.ClassViewHolder> {

    private List<ScoreCard.ScoreParameter> classList;
    private int selectedPosition = -1;
    private OnDocUploadClickListener uploadClickListener;
    public DependenciesAdapter(List<ScoreCard.ScoreParameter> classList, OnDocUploadClickListener listener) {
    ...
    @NonNull
    @Override
    public DependenciesAdapter.ClassViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.adapter_applicant_age_item, parent, false);
        return new DependenciesAdapter.ClassViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull DependenciesAdapter.ClassViewHolder holder, @SuppressLint("RecyclerView") int position) {
