...
public class BankRelationAdapter extends RecyclerView.Adapter<BankRelationAdapter.ClassViewHolder> {

    private List<ScoreCard.ScoreParameter> classList;
    private int selectedPosition = -1;
    private OnDocUploadClickListener uploadClickListener;

    public BankRelationAdapter(List<ScoreCard.ScoreParameter> classList, OnDocUploadClickListener listener) {
    ...
    @NonNull
    @Override
    public BankRelationAdapter.ClassViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.adapter_applicant_age_item, parent, false);
        return new BankRelationAdapter.ClassViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull BankRelationAdapter.ClassViewHolder holder, @SuppressLint("RecyclerView") int position) {
