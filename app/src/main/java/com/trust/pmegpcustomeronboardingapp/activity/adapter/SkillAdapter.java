...
public class SkillAdapter extends RecyclerView.Adapter<SkillAdapter.ClassViewHolder> {

    private List<ScoreCard.ScoreParameter> classList;
    private int selectedPosition = -1;
    private OnDocUploadClickListener uploadClickListener;
    public SkillAdapter(List<ScoreCard.ScoreParameter> classList, OnDocUploadClickListener listener) {
    ...
    @NonNull
    @Override
    public SkillAdapter.ClassViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.adapter_applicant_age_item, parent, false);
        return new SkillAdapter.ClassViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull SkillAdapter.ClassViewHolder holder, @SuppressLint("RecyclerView") int position) {
