...
public class ActivityAdapter extends RecyclerView.Adapter<ActivityAdapter.ClassViewHolder> {

    private List<ScoreCard.ScoreParameter> classList;
    private int selectedPosition = -1;
    private OnDocUploadClickListener uploadClickListener;
    public ActivityAdapter(List<ScoreCard.ScoreParameter> classList, OnDocUploadClickListener listener) {
    ...
    @NonNull
    @Override
    public ActivityAdapter.ClassViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.adapter_applicant_age_item, parent, false);
        return new ActivityAdapter.ClassViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull ActivityAdapter.ClassViewHolder holder, @SuppressLint("RecyclerView") int position) {
