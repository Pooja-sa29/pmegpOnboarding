...
public class HouseAdapter extends RecyclerView.Adapter<HouseAdapter.ClassViewHolder> {

    private List<ScoreCard.ScoreParameter> classList;
    private int selectedPosition = -1;
    private OnDocUploadClickListener uploadClickListener;

    public HouseAdapter(List<ScoreCard.ScoreParameter> classList,OnDocUploadClickListener listener) {
    ...
    @NonNull
    @Override
    public HouseAdapter.ClassViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.adapter_applicant_age_item, parent, false);
        return new HouseAdapter.ClassViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull HouseAdapter.ClassViewHolder holder, @SuppressLint("RecyclerView") int position) {
