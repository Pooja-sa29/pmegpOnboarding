...
public class GovtRegAdapter extends RecyclerView.Adapter<GovtRegAdapter.ClassViewHolder> {

    private List<ScoreCard.ScoreParameter> classList;
    private int selectedPosition = -1;
    private OnDocUploadClickListener uploadClickListener;
    public GovtRegAdapter(List<ScoreCard.ScoreParameter> classList, OnDocUploadClickListener listener) {
    ...
    @NonNull
    @Override
    public GovtRegAdapter.ClassViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.adapter_applicant_age_item, parent, false);
        return new GovtRegAdapter.ClassViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull GovtRegAdapter.ClassViewHolder holder, @SuppressLint("RecyclerView") int position) {
