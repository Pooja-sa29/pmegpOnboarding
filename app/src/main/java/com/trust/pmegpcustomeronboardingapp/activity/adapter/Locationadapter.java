...
public class Locationadapter extends RecyclerView.Adapter<Locationadapter.ClassViewHolder> {

    private List<ScoreCard.ScoreParameter> classList;
    private int selectedPosition = -1;
    private OnDocUploadClickListener uploadClickListener;

    public Locationadapter(List<ScoreCard.ScoreParameter> classList, OnDocUploadClickListener listener) {
    ...
    @NonNull
    @Override
    public Locationadapter.ClassViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.adapter_applicant_age_item, parent, false);
        return new Locationadapter.ClassViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull Locationadapter.ClassViewHolder holder, @SuppressLint("RecyclerView") int position) {
