...
public class MarketingAdapter extends RecyclerView.Adapter<MarketingAdapter.ClassViewHolder> {

    private List<ScoreCard.ScoreParameter> classList;
    private int selectedPosition = -1;
    private OnDocUploadClickListener uploadClickListener;
    public MarketingAdapter(List<ScoreCard.ScoreParameter> classList, OnDocUploadClickListener listener) {
    ...
    @NonNull
    @Override
    public MarketingAdapter.ClassViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.adapter_applicant_age_item, parent, false);
        return new MarketingAdapter.ClassViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull MarketingAdapter.ClassViewHolder holder, @SuppressLint("RecyclerView") int position) {
