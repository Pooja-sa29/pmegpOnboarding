...
public class IncomeAdapter extends RecyclerView.Adapter<IncomeAdapter.ClassViewHolder> {

    private List<ScoreCard.ScoreParameter> classList;
    private int selectedPosition = -1;
    private OnDocUploadClickListener uploadClickListener;

    public IncomeAdapter(List<ScoreCard.ScoreParameter> classList, OnDocUploadClickListener listener) {
    ...
    @NonNull
    @Override
    public IncomeAdapter.ClassViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.adapter_applicant_age_item, parent, false);
        return new IncomeAdapter.ClassViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull IncomeAdapter.ClassViewHolder holder, @SuppressLint("RecyclerView") int position) {
