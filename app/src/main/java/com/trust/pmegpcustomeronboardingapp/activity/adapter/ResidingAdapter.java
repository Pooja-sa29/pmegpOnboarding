...
public class ResidingAdapter extends RecyclerView.Adapter<ResidingAdapter.ClassViewHolder> {

    private List<ScoreCard.ScoreParameter> classList;
    private int selectedPosition = -1;
    private OnDocUploadClickListener uploadClickListener;

    public ResidingAdapter(List<ScoreCard.ScoreParameter> classList,OnDocUploadClickListener listener) {
    ...
    @NonNull
    @Override
    public ResidingAdapter.ClassViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.adapter_applicant_age_item, parent, false);
        return new ResidingAdapter.ClassViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull ResidingAdapter.ClassViewHolder holder, @SuppressLint("RecyclerView") int position) {
