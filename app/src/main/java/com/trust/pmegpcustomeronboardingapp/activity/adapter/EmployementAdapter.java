...
public class EmployementAdapter extends RecyclerView.Adapter<EmployementAdapter.ClassViewHolder> {

    private List<ScoreCard.ScoreParameter> classList;
    private int selectedPosition = -1;
    private OnDocUploadClickListener uploadClickListener;
    public EmployementAdapter(List<ScoreCard.ScoreParameter> classList, OnDocUploadClickListener listener) {
    ...
    @NonNull
    @Override
    public EmployementAdapter.ClassViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.adapter_applicant_age_item, parent, false);
        return new EmployementAdapter.ClassViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull EmployementAdapter.ClassViewHolder holder, @SuppressLint("RecyclerView") int position) {
